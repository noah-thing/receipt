...
program2.name("receipt").description("See exactly what your AI coding agent cost \u2014 itemized on every pull request.").version("0.4.0");
function repoRoot() {
  return findRepoRoot();
}
...
program2.command("health").alias("session").description("Is the current session still sharp? Warns before context degrades.").action(() => {
  const root = repoRoot();
  const now = Date.now();
