...
function usageSummaryText(receipt, fuel2, extras = {}) {
  ...
  if (extras.topTip) {
    out.push(import_picocolors2.default.dim(`   \u{1F4A1} ${extras.topTip.title}${extras.topTip.impact ? ` (${extras.topTip.impact})` : ""}`));
  }
  if (extras.fun) {
    const eq = funEquivalences(receipt.totalTokens, extras.repoTokens);
...
var SEV_MARK = {
  high: (s) => import_picocolors2.default.red("\u203C " + s),
  medium: (s) => import_picocolors2.default.yellow("\u26A0 " + s),
  low: (s) => import_picocolors2.default.dim("\xB7 " + s)
};
...
function sigMark(sev) {
  if (sev === "high") return (s) => import_picocolors2.default.red("\u{1F534} " + s);
  if (sev === "watch") return (s) => import_picocolors2.default.yellow("\u{1F7E1} " + s);
  return (s) => import_picocolors2.default.dim("\xB7 " + s);
}
function renderHealth(h, tax, currency = "USD") {
  const out = [];
  out.push("");
  out.push(import_picocolors2.default.bold("\u{1F9E0} Session health \u2014 keeping the agent sharp"));
  out.push("");
  if (!h) {
    out.push(import_picocolors2.default.dim("No recent session in the ledger yet. Run your agent (or import it), then check back."));
    out.push("");
    return out.join("\n");
  }
  const head = `${STATUS_BADGE[h.status]}  ` + import_picocolors2.default.dim(
  ...
  if (h.signals.length === 0) {
    out.push(import_picocolors2.default.green("  Looking sharp. Nothing to refresh yet."));
  } else {
    for (const s of h.signals) {
      out.push("  " + sigMark(s.severity)(s.detail));
      out.push(import_picocolors2.default.cyan("     \u2192 " + s.action));
      out.push("");
    }
  }
  if (tax) {
...
function renderContextTax(t, currency = "USD") {
  ...
  out.push("  " + import_picocolors2.default.bold(`\u{1F4E6} Re-sent context: ${tag} ${sharePct}% of this session's tokens`));
  out.push(
    import_picocolors2.default.dim(
...
function renderHealthHistory(rows, limit = 12, profile) {
  const out = [];
  out.push("");
  out.push(import_picocolors2.default.bold("\u{1F9E0} Session history \u2014 how your sessions have held up"));
  out.push("");
  if (rows.length === 0) {
    out.push(import_picocolors2.default.dim("No sessions in the ledger yet. Run your agent (or import it), then check back."));
    out.push("");
    return out.join("\n");
  }
  const shown = rows.slice(-limit).reverse();
  out.push(import_picocolors2.default.dim("  #    when             turns   peak ctx   ~compact   verdict"));
  for (const r of shown) {
    ...
    const verdict = r.compactedLate ? import_picocolors2.default.yellow("compacted late") : h.status === "fresh" ? import_picocolors2.default.dim("fresh-ish") : h.status === "healthy" ? import_picocolors2.default.green("healthy") : import_picocolors2.default.yellow(h.status);
    out.push(
...
function degradationProfileLine(p) {
  ...
  return parts.length ? import_picocolors2.default.dim("  " + parts.join("  ")) : "";
}
function renderAdvice(receipt, recs) {
  const out = [];
  out.push("");
  out.push(import_picocolors2.default.bold("\u{1F4A1} Advice \u2014 cut the waste, keep the quality"));
  out.push("");
  if (receipt.totalTokens === 0) {
    out.push(import_picocolors2.default.dim("No usage recorded for this scope yet. Run your agent, then check back."));
    out.push("");
    return out.join("\n");
  }
  const drivers = costDrivers(receipt);
  if (drivers.length) {
    out.push(import_picocolors2.default.bold("What's driving the cost"));
    for (const d of drivers) out.push("  \u2022 " + d.replace(/`/g, ""));
    out.push("");
  }
  out.push(import_picocolors2.default.bold("Recommendations"));
  for (const r of recs) {
    const mark = SEV_MARK[r.severity] ?? ((s) => s);
    out.push("  " + mark(r.title) + (r.impact ? import_picocolors2.default.green(`   ${r.impact}`) : ""));
    out.push(import_picocolors2.default.dim(wrap(r.detail.replace(/`/g, ""), 78, "     ")));
    out.push("");
  }
  return out.join("\n");
}
...
program2.name("receipt").description("See exactly what your AI coding agent cost \u2014 itemized on every pull request.").version("0.5.0");
function repoRoot() {
  return findRepoRoot();
}
function drainStdin() {
...
program2.command("fuel").description("How much of your plan you're using right now, and what's left.").option("--fun", "include playful real-world equivalences").action((opts) => {
  ...
  const fun = Boolean(opts.fun) || config.fun === true;
  let out = renderFuel(f, now, { fun, repoTokens: fun ? estimateRepoTokens(root) : void 0 });
  const h = sessionHealth(entries, now);
  if (h) out += healthOneLine(h) + import_picocolors3.default.dim("  (receipt health for detail)") + "\n\n";
  process.stdout.write(out);
});
...
program2.command("advice").alias("tips").description("How to cut wasted tokens \u2014 without making the work worse.").option("--branch <branch>", "branch to scope to (default: current)").option("--base <base>", "base branch to diff against", "main").option("--all", "analyze the whole ledger, ignoring branch").option("--today", "only today's calls").action((opts) => {
  const root = repoRoot();
  const receipt = computeReceipt(root, {
    branch: opts.all ? void 0 : opts.branch,
    base: opts.base,
...
program2.command("health").alias("session").description("Is the current session still sharp? Warns before context degrades.").option("--all", "Show every past session and your degradation profile, not just the current one.").option("-n, --limit <n>", "How many sessions to show with --all.", "12").option("--json", "Machine-readable output (the SessionHealth object).").option("-q, --quiet", "No output; signal only via exit code.").option("--gate <status>", "Non-zero exit at/above this status: watch|degrading|critical.", "degrading").action((opts) => {
  const root = repoRoot();
  const now = Date.now();
  const entries = readLedger(ledgerPath(root));
  if (opts.all) {
  ...
  if (opts.json) {
    ...
    const tax = last ? contextTax(last, Pricing.load(root)) : void 0;
    process.stdout.write(renderHealth(h, tax, loadConfig(root).currency ?? "USD"));
  }
  process.exitCode = healthExitCode(h, opts.gate);
});
program2.command("guard").description("Hook entrypoint: nudge the agent the moment a session starts to degrade.").option("--gate <status>", "Fire at/above this status: watch|degrading|critical.", "watch").option("--notify", "Exit 2 so Claude Code surfaces the message to the agent.").action(async (opts) => {
  await drainStdin();
  const root = repoRoot();
  const h = sessionHealth(readLedger(ledgerPath(root)), Date.now());
  const code = healthExitCode(h, opts.gate);
  if (!h || code === 0) return;
